...
function Read-Json([string] $Path) {
    if (-not [System.IO.File]::Exists($Path)) {
        throw "Required analysis pipeline corpus file is missing: $Path"
    }
...
if ($declaredPaths.Count -ne 10 -or $declaredPaths.Count -ne (@($declaredPaths | Select-Object -Unique)).Count) {
    throw 'analysis pipeline corpus package_file_paths must be an exact ten-file unique closure.'
}
...
if (($actualPackagePaths -join "`n") -cne ($expectedPackagePaths -join "`n")) {
    throw 'analysis pipeline corpus on-disk package closure differs from package_file_paths.'
}
...
foreach ($file in @($manifest.files)) {
    ...
    if ($item.Length -ne [long] $file.bytes -or $sha -cne [string] $file.sha256) {
        throw "analysis pipeline corpus frozen file identity mismatch: $($file.path)"
    }
...
if (@($manifest.files).Count -ne 9 -or @($manifest.files.path) -contains 'fixture-manifest.v1.json') {
    throw 'analysis pipeline corpus hash-bound file list must contain exactly the nine non-self files.'
}
...
if ($aggregateSha -cne [string] $manifest.content_aggregate.sha256) {
    throw 'analysis pipeline corpus ordered content aggregate does not match the manifest.'
}

if (-not (Test-Json -LiteralPath $ordinaryPath -SchemaFile $schemaPath -ErrorAction Stop)) {
    throw 'analysis pipeline corpus ordinary input failed its recursively closed pre-dispatch schema.'
}
...
if ($ordinaryClassDrifted -or $oracleUsedProductOutput -or $oracleAuthorshipDrifted) {
    throw 'analysis pipeline corpus answer-isolation metadata is not closed.'
}
...
if ($caseIds.Count -ne 4 -or ($caseIds -join "`n") -cne ($oracleCaseIds -join "`n")) {
    throw 'analysis pipeline corpus harness and oracle must bind the same four ordered cases.'
}
$registrations = @($manifest.accumulated_package_registrations)
if ($registrations.Count -ne 11) {
    throw 'analysis pipeline corpus must register the exact two documentation, three candidate-analysis, four finding/case, and two operational package identities.'
}
...
if ($authorityRevision -cne 'e7de0305515657223c513195f8323b2649b6c7c8') {
    throw 'analysis pipeline corpus source authority is not bound to the accepted starting revision.'
}
foreach ($authority in @($manifest.source_authority)) {
    ...
    if ([System.IO.Path]::IsPathRooted($authorityPath) -or
        $authorityPath -match '(^|[\\/])\.\.([\\/]|$)') {
        throw "analysis pipeline corpus source authority path is unsafe: $authorityPath"
    }
    ...
    if (($authorityBytes.LongLength -ne [long] $authority.bytes) -or
        ...
        throw "analysis pipeline corpus frozen source authority identity does not resolve at the accepted revision: $authorityPath"
    }
}
...
if (@($directIds | Where-Object { $overstated -contains $_ }).Count -ne 0) {
    throw 'analysis pipeline corpus direct four-case ownership retains an unexercised scale/Bethesda/desktop/budget claim.'
}
...
foreach ($id in $overstated) {
    if ($inheritedIds -notcontains $id) {
        throw "analysis pipeline corpus inherited evidence scope is missing $id."
    }
}

if (-not [System.IO.File]::Exists($reviewPath)) {
    throw 'analysis pipeline corpus final independent review is missing.'
}
...
$manifestStatusDrifted = [string] $manifest.status -cne 'independently-reviewed-accepted-functional-naming-revalidation'
...
if ($manifestStatusDrifted -or $reviewVerdictMissing -or $reviewManifestMissing -or $reviewAggregateMissing -or $reviewLengthMissing) {
    throw 'analysis pipeline corpus final independent review does not externally freeze and accept the exact manifest/aggregate.'
}
