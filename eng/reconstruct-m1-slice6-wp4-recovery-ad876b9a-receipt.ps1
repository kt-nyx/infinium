...
$isE6Recovery=$ManifestId-ceq'infinium.m1-s6.wp4.credential-native-recovery/6232bae5-f735-4db7-a74f-7ede9f67b752'
if(-not$isAdRecovery-and-not$isE3Recovery-and-not$isE6Recovery){throw 'Current recovery reconstruction identity differs.'}
if($isE6Recovery){
    $expectedPriorEvidenceSha='5b565888a412188f7c814c0d923e696e27d4135d7ebb23f5884ef7b2e3f228c7'
    $expectedPriorLockSha='4fc808d221d340eb6b145ceffa35a2472cd621102b0e0dc280a8dbb71f77ddd4'
    $expectedFailedManifestId='infinium.m1-s6.wp4.credential-native-authorization/e6e04651-4cd5-4f5d-8b46-5ec84a81cbbe'
    $expectedPriorFingerprints=@()
    $priorExactAbsenceCount=0
    $recoveryTargetAbsenceCount=12
}elseif($isE3Recovery){
...
if(-not$TestOnlyPaths){
    if($isE6Recovery){$expected=@(
        [IO.Path]::GetFullPath((Join-Path $root 'docs/plans/milestones/m1/slices/s6/wp4-credential-native-recovery.e6e04651.v1.json')),
        [IO.Path]::GetFullPath((Join-Path $root 'artifacts/m1-slice6/wp4-native-recovery-6232bae5/credential-native-recovery-evidence.v1.json')),
        [IO.Path]::GetFullPath((Join-Path $root 'artifacts/m1-slice6/wp4-native-recovery-locks/58a7f448813901441cd5c14a9b78c04e1d32c9ca068037c620f39387417dd63d.json')),
        [IO.Path]::GetFullPath((Join-Path $root 'artifacts/m1-slice6/wp4-native-e6e04651/credential-native-cleanup-ambiguity.v2.json')),
        [IO.Path]::GetFullPath((Join-Path $root 'artifacts/m1-slice6/wp4-native-authority-locks/49073f973272d97446b1d4462af86d2e68fe117ab48119e54a4e5dbfb59ba23c.json')),
        [IO.Path]::GetFullPath((Join-Path $root 'artifacts/m1-slice6/wp4-native-recovery-6232bae5/CredentialNativeRecovery.json')))
    }elseif($isE3Recovery){$expected=@(
...
$priorFingerprints = if ($prior.PSObject.Properties.Name -contains 'absence_target_fingerprints') {
    @($prior.absence_target_fingerprints)
}
else {
    @()
}
if($isE6Recovery){
    if($prior.status-cne'failed-cleanup-ambiguous'-or$prior.manifest_id-cne$expectedFailedManifestId-or
        [int]$prior.later_native_calls-ne0-or[bool]$prior.cleanup_confirmed-or
        [bool]$prior.whole_namespace_absence_confirmed-or-not[bool]$prior.namespace_blocked-or
        $prior.namespace_disposition-cne'consumed-never-reuse'-or
        $prior.assignment_id-cne'wp4-v2/backup-restore-reauthentication/cleanup-successor'-or
        $prior.reason-cne'cleanup-phase-failed'-or$prior.PSObject.Properties.Name-contains'absence_target_fingerprints'-or
        $prior.PSObject.Properties.Name-contains'target_absence'){throw 'Prior cleanup ambiguity evidence differs.'}
}elseif($prior.status-cne'failed-primary-cleanup-confirmed'-or$prior.manifest_id-cne$expectedFailedManifestId-or
...
$value=[ordered]@{gate='CredentialNativeRecovery';status='passed';network_permitted=$false;credential_access_permitted=$true;evidence=[ordered]@{
    ...
    prior_exact_absence_count=$priorExactAbsenceCount;prior_terminal_evidence_sha256=$PriorEvidenceSha256;provider_operations=0
    receipt_origin='post-effect-reconstruction-from-immutable-evidence-no-native-retry';recovery_target_absence_count=$recoveryTargetAbsenceCount}}
