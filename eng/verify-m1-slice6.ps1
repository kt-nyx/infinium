...
param(
    ...
    [switch] $M1Slice6CampaignReview,

    [switch] $M1Slice6CampaignReviewCloseout,

    [switch] $M1Slice6CampaignAdmissionCloseout,
...
if ($Gate -in @('Layer6Review', 'CredentialNative', 'CredentialNativeRecovery', 'SourceClaimSemantics', 'CandidateSemantics', 'ProductSemanticConformance', 'ProvenanceReplay', 'LiveEvidence', 'RetainedReplay', 'ComposedProvenance', 'HistoricalSemanticPackageIntegrity', 'CampaignV2NonLive', 'C1Readiness', 'NonLiveAll') -and $PSVersionTable.PSEdition -ne 'Core') {
    ...
    if ($M1Slice6CampaignReview) { $arguments += '-M1Slice6CampaignReview' }
...
function Get-Wp8NonLiveCurrentStateDisposition(
    [string] $CurrentStateText,
    [object] $AcceptanceBinding,
    [string] $ReadmeText = '',
    [string] $RecordText = '',
    [object] $Wp9ReviewBinding = $null) {
    ...
    $wp9OwnerStopSourceLinkCorrection =
        ...
        $wp9NoEffect -and $noInheritance
    $wp9ReviewCloseoutCorrection =
        $AcceptanceBinding.state -eq 'accepted-closeout' -and
        $normalized.Contains('| Current authorized work | `M1/S6/WP9` bounded non-effectful review-closeout correction and reverification only.', [StringComparison]::Ordinal) -and
        $normalized.Contains('The B14 reviewed-pending-owner transition exposed stale retained-WP8 and Layer 6 closeout predicates.', [StringComparison]::Ordinal) -and
    ...
    if ($AcceptanceBinding.state -eq 'accepted-closeout' -and $null -ne $Wp9ReviewBinding -and
        ...
        [string]$Wp9ReviewBinding.close_ready_commit -match '^[0-9a-f]{40}$' -and
        [string]$Wp9ReviewBinding.reviewed_candidate_commit -match '^[0-9a-f]{40}$') {
        ...
        $wp9ReviewedOwnerPending =
            (Test-Wp9DocumentationRequirements -CurrentStateText $CurrentStateText `
                -ReadmeText $ReadmeText -Requirements $reviewRequirements) -and
...
function Get-Wp9ReviewBindingForCandidate([string] $CandidateCommit) {
    ...
    $reviewPattern = '^WP9_PROFILE_REVIEW_ACCEPTANCE candidate_commit=([0-9a-f]{40}) manifest_id=' +
        [Regex]::Escape([string]$manifest.manifest_id) + ' sha256=' +
        [Regex]::Escape($manifestSha) + ' verdicts=security,semantics,diff$'
    $matchingReviewLines = @($recordText -split "`n" | Where-Object { [Regex]::IsMatch($_, $reviewPattern) })
    if ($matchingReviewLines.Count -ne 1) { return $null }
    $reviewedCandidate = [Regex]::Match($matchingReviewLines[0], $reviewPattern).Groups[1].Value
    & git -C $repoRoot merge-base --is-ancestor $reviewedCandidate $CandidateCommit
    if ($LASTEXITCODE -ne 0) { return $null }
    $reviewedRecord = Get-CandidateText $reviewedCandidate 'docs/plans/milestones/m1/slices/s6/record.md'
    $closeoutPaths = @(& git -C $repoRoot -c core.quotePath=false diff --name-only $reviewedCandidate $CandidateCommit --)
    if ($LASTEXITCODE -ne 0) { return $null }
...
function Test-M1Slice6RemainderR1Candidate([string] $CandidateCommit) {
    ...
    return Test-M1Slice6RemainderR1NoEffectState `
        (Get-M1Slice6CandidateBlobText $CandidateCommit 'docs/current-state.md') `
        (Get-M1Slice6CandidateBlobText $CandidateCommit 'docs/plans/milestones/m1/slices/s6/README.md') `
        (Get-M1Slice6CandidateBlobText $CandidateCommit 'docs/plans/milestones/m1/slices/s6/record.md')
}
...
function Invoke-Layer6ReviewGate(
    ...
    if ($CampaignReviewMode) {
        if ($Wp9OwnerStopMode -or $Wp8PreLiveCloseoutMode -or $HandoffCloseout -or $Wp4OwnerReviewHandoff -or
            ...
            $CampaignAdmissionCloseoutMode -or $CampaignRolloverCloseoutMode) {
            throw 'M1Slice6CampaignReview cannot be combined with another Layer6 authority mode.'
        }
        $head = (& git -C $repoRoot rev-parse HEAD).Trim()
        $manifestPath = Join-Path $repoRoot 'docs/plans/milestones/m1/slices/s6/m1-slice6-finite-campaign-authorization.v1.json'
        $validationJson = @(& (Join-Path $repoRoot 'eng/validate-m1-slice6-campaign.ps1') `
            -AuthorizationManifest $manifestPath -RequireState Ready) -join "`n"
    ...
    if ($Wp9ReviewCloseoutMode) {
        ...
        $currentText = Get-CandidateText $candidateHash 'docs/current-state.md'
        $readmeText = Get-CandidateText $candidateHash 'docs/plans/milestones/m1/slices/s6/README.md'
        $recordText = Get-CandidateText $candidateHash 'docs/plans/milestones/m1/slices/s6/record.md'
        $matrix = Get-CandidateText $candidateHash 'docs/plans/milestones/m1/slices/s6/wp8-case-requirement-matrix.v1.json' | ConvertFrom-Json -Depth 100
        $wp9CurrentStateDisposition = Get-Wp8NonLiveCurrentStateDisposition `
            $currentText $matrix.acceptance_binding $readmeText $recordText $reviewBinding
        ...
        $reviewCommitCount = (& git -C $repoRoot rev-list --count "$baselineHash..$candidateHash").Trim()
        if (-not (Test-Wp9ReviewCloseoutLayer6Transition `
                $baselineHash $candidateHash $head $reviewBinding $wp9CurrentStateDisposition `
                $reviewCommitCount $actualCloseoutPaths)) {
            throw 'Wp9ReviewCloseout requires one exact three-document reviewed-pending-owner transition from the exact reviewed candidate.'
        }
        $validationJson = @(& (Join-Path $repoRoot 'eng/validate-m1-slice6-wp9-profile-authorization.ps1') `
            -AuthorizationManifest (Join-Path $repoRoot 'docs/plans/milestones/m1/slices/s6/wp9-production-profile-authorization.v1.json') -RequireReady) -join "`n"
        if ($LASTEXITCODE -ne 0 -or ($validationJson | ConvertFrom-Json -Depth 20).status -cne 'validated-ready-for-owner-acceptance') {
            throw 'Wp9ReviewCloseout requires the unchanged exact ready manifest.'
        }
    ...
    if ($CampaignReviewMode) {
        [string[]]$actualCampaignPaths = @($changedPaths | ForEach-Object { [string]$_.path })
        [Array]::Sort($actualCampaignPaths, [StringComparer]::Ordinal)
        [Array]::Sort($campaignReviewPaths, [StringComparer]::Ordinal)
        if ([string]::Join("`n", $actualCampaignPaths) -cne [string]::Join("`n", $campaignReviewPaths)) {
    ...
    if ($Wp9ReviewCloseoutMode) {
        [string[]]$actualReviewCloseoutPaths = @($changedPaths | ForEach-Object { [string]$_.path })
        [Array]::Sort($actualReviewCloseoutPaths, [StringComparer]::Ordinal)
        [Array]::Sort($wp9ReviewCloseoutPaths, [StringComparer]::Ordinal)
        if ([string]::Join("`n", $actualReviewCloseoutPaths) -cne [string]::Join("`n", $wp9ReviewCloseoutPaths) -or
            $wp9CurrentStateDisposition -cne 'exact-wp9-reviewed-owner-pending-no-effect-state') {
            $failures.Add('Wp9ReviewCloseout requires exactly current-state, Slice 6 README, and append-only record in the exact reviewed-pending-owner state.')
        }
    }
    if ($Wp4OwnerReviewHandoff) {
        $currentState = @($changedPaths | Where-Object { $_.path -ceq 'docs/current-state.md' })
    ...
    Write-Receipt 'Layer6Review' ([ordered]@{
        ...
        m1_slice6_campaign_review = [bool]$CampaignReviewMode
...
function Invoke-StateSurfaceGate([bool] $RequireAcceptedInputProof) {
    ...
    if ($traceability.maturity -ne 'Slice-frozen') {
        throw 'Owner-accepted Slice 6 traceability must remain Slice-frozen.'
    }
...
function Invoke-NonLiveAllGate {
    ...
    $remainderR1State = Test-M1Slice6RemainderR1NoEffectState $currentState $sliceReadme $recordText
    if ($currentStateDisposition -eq 'invalid' -and $remainderR1State -and
        (Test-M1Slice6RemainderR1Candidate $head)) {
        $currentStateDisposition = 'exact-m1-s6-remainder-r1-no-effect-state'
    }
    if ($currentStateDisposition -eq 'invalid' -and $campaignState) {
        ...
        if ($LASTEXITCODE -ne 0 -or ($campaignValidationJson | ConvertFrom-Json -Depth 20).disposition -cne 'ready') {
            throw 'NonLiveAll exact finite-campaign state failed its ready campaign validator.'
        }
        $currentStateDisposition = 'exact-m1-s6-finite-campaign-review-no-effect-state'
    }
    ...
    if ($currentStateDisposition -in @(
            'exact-wp9-profile-owner-stop-no-effect-state',
            'exact-wp9-reviewed-owner-pending-no-effect-state')) {
        ...
        $closeoutBaseline = if ($currentStateDisposition -eq 'exact-wp9-reviewed-owner-pending-no-effect-state') {
            [string]$wp9ReviewBinding.reviewed_candidate_commit
        } else { $closeReady }
        [string[]]$actualCloseoutPaths = @(& git -C $repoRoot diff --name-only "$closeoutBaseline..$head")
        [string[]]$expectedCloseoutPaths = if ($currentStateDisposition -eq 'exact-wp9-reviewed-owner-pending-no-effect-state') {
            @('docs/current-state.md','docs/plans/milestones/m1/slices/s6/README.md','docs/plans/milestones/m1/slices/s6/record.md')
        } else {
            @('docs/current-state.md','docs/plans/milestones/m1/slices/s6/README.md','docs/plans/milestones/m1/slices/s6/record.md','docs/plans/milestones/m1/slices/s6/wp9-production-profile-authorization.v1.json')
        }
        [Array]::Sort($actualCloseoutPaths, [StringComparer]::Ordinal)
        [Array]::Sort($expectedCloseoutPaths, [StringComparer]::Ordinal)
        if ((& git -C $repoRoot rev-list --count "$closeoutBaseline..$head").Trim() -ne '1' -or
            [string]::Join("`n", $actualCloseoutPaths) -cne [string]::Join("`n", $expectedCloseoutPaths)) {
            throw 'NonLiveAll exact WP9 state requires one exact state-specific closeout commit with no extra paths.'
        }
    }
...
function Invoke-CredentialNativeRecoveryGate {
    ...
    if([string]::Equals($path,$expected076b,[StringComparison]::OrdinalIgnoreCase)){
        ...
        $priorEvidencePath=Join-Path $repoRoot 'artifacts/m1-slice6/wp4-native-4936dcef/credential-native-cleanup-ambiguity.v3.json'
        $priorLockPath=Join-Path $repoRoot 'artifacts/m1-slice6/wp4-native-authority-locks/16d19410cd200caee29da362c474805929cc4c65651685173d39838849e27421.json'
        $validatorPath=Join-Path $repoRoot 'eng/validate-m1-slice6-wp4-recovery-4936dcef.ps1'
        $expectedPriorFingerprints=@(
            '1280f6eb54fc26043b11c19b40f8d5fff0777aeb3ea6b8ad3d225365b1b41677','1b83b44086e5ef8d7173acc772da1a471924a5e7fd53bfefe9e85f882d282275',
            'ce92785c4620f0482503a0cfc282feb87e065827735ad4b5d2645c448b5b9f5f','d16fcd6b08b0d3eeb4931b8d440a5e5a245000692343e9e0ce57a415b1e2915e',
            '5a63263824e2d065203489bbf18615a6324d8493a153e57d98a9021947d19e9c','ebd23333c28423c14099f86ae43b294e9c0911359b8df8051792ed261d86811a',
            '4b57de8f5cf207ca7fd25a99582a7e9a9c65bd0756570312d2211df9c2de65e8','eb5044984f24f3a4a56a016a24f50245659d270f20bb31ce6c2752b3d75aba2a',
            '6d8306b7661f2b3242ad93c2438917fac74cf93b1e52c486e95ff346550d37bb','13df4cf0001cd0571f9b05463d0bf9e61016480b1eb637b7c99378bd584fb09e',
            'e995b83b9d65f4d8d1f07ae59d03d6af31fc90275ed48a90e059106a7d388900')
        $priorExactAbsenceCount=11
        $recoveryTargetCount=1
        $expectedPriorStatus='failed-cleanup-ambiguous-v3'
    }elseif([string]::Equals($path,$expectedE6,[StringComparison]::OrdinalIgnoreCase)){
        $expectedManifestId='infinium.m1-s6.wp4.credential-native-recovery/6232bae5-f735-4db7-a74f-7ede9f67b752'
        $failureRecordCommit='0d0b230c73ddd5bb11f4b0f8cf3b85a5cee2a82d'
        $manifestRelative='docs/plans/milestones/m1/slices/s6/wp4-credential-native-recovery.e6e04651.v1.json'
        $priorEvidencePath=Join-Path $repoRoot 'artifacts/m1-slice6/wp4-native-e6e04651/credential-native-cleanup-ambiguity.v2.json'
        $priorLockPath=Join-Path $repoRoot 'artifacts/m1-slice6/wp4-native-authority-locks/49073f973272d97446b1d4462af86d2e68fe117ab48119e54a4e5dbfb59ba23c.json'
        $validatorPath=Join-Path $repoRoot 'eng/validate-m1-slice6-wp4-recovery-e6e04651.ps1'
        $expectedPriorFingerprints=@()
        $priorExactAbsenceCount=0
        $recoveryTargetCount=12
        $expectedPriorStatus='failed-cleanup-ambiguous'
    }elseif([string]::Equals($path,$expectedE3,[StringComparison]::OrdinalIgnoreCase)){
        ...
        $priorExactAbsenceCount=10
        $recoveryTargetCount=2
        $expectedPriorStatus='failed-primary-cleanup-confirmed'
    ...
    if($expectedPriorStatus-eq'failed-evidence-retention'){
        ...
        if($priorLock.manifest_id-ne[string]$m.binding.failed_manifest_id-or
            ...
            $priorLock.disposition-ne'consumed-before-native-launch-never-delete-or-reuse'){
            throw 'Recovery consumed 4936dcef authority lock content differs; no effect is permitted.'
        }
        if($priorEvidence.schema-ne'infinium.m1-s6.wp4.credential-native-cleanup-ambiguity/v3'-or
            $priorEvidence.status-ne'failed-cleanup-ambiguous'-or
            ...
            [int]$priorEvidence.later_native_calls-ne0-or[bool]$priorEvidence.cleanup_confirmed-or
            [bool]$priorEvidence.whole_namespace_absence_confirmed-or-not[bool]$priorEvidence.namespace_blocked-or
            $priorEvidence.namespace_disposition-ne'consumed-never-reuse'-or
            $priorEvidence.assignment_id-ne'wp4-v2/backup-restore-reauthentication/cleanup-successor'-or
            $priorEvidence.reason-ne'cleanup-phase-failed'-or
            $priorEvidence.prior_primary_failure.failure_type-ne'CredentialNativeHelperEvidenceAmbiguityException'-or
            $priorEvidence.terminal_failure.failure_type-ne'InvalidDataException'-or
            [int]$m.binding.prior_exact_absence_count-ne11-or
            (($m.binding.prior_exact_absence_fingerprints-join'|')-cne($expectedPriorFingerprints-join'|'))-or
            @($m.disposable_namespace.targets.target_fingerprint_sha256|?{$_ -in $expectedPriorFingerprints}).Count-ne0){
            throw 'Recovery prior 4936dcef ambiguity lineage is not exact; no effect is permitted.'
        }
    }elseif($expectedPriorStatus-eq'failed-cleanup-ambiguous'){
        if($priorLock.manifest_id-ne[string]$m.binding.failed_manifest_id-or
            $priorLock.manifest_sha256-ne[string]$m.binding.failed_manifest_sha256-or
            $priorLock.execution_head_commit-ne'fdd9d49301e72f6a421e4597ea405bb2ca69da2f'-or
            $priorLock.disposition-ne'consumed-before-native-launch-never-delete-or-reuse'){
            throw 'Recovery consumed authority lock content differs; no effect is permitted.'
        }
        if($priorEvidence.status-ne$expectedPriorStatus-or
            $priorEvidence.manifest_id-ne[string]$m.binding.failed_manifest_id-or
            $priorEvidence.manifest_sha256-ne[string]$m.binding.failed_manifest_sha256-or
            [int]$priorEvidence.later_native_calls-ne0-or[bool]$priorEvidence.cleanup_confirmed-or
            [bool]$priorEvidence.whole_namespace_absence_confirmed-or-not[bool]$priorEvidence.namespace_blocked-or
            ...
            $priorEvidence.assignment_id-ne'wp4-v2/backup-restore-reauthentication/cleanup-successor'-or
            $priorEvidence.reason-ne'cleanup-phase-failed'-or
            $priorEvidence.PSObject.Properties.Name-contains'absence_target_fingerprints'-or
            $priorEvidence.PSObject.Properties.Name-contains'target_absence'){
            throw 'Recovery prior ambiguity lineage is not exact; no effect is permitted.'
        }
    }elseif($priorEvidence.status-ne'failed-primary-cleanup-confirmed'-or
        $priorEvidence.manifest_id-ne[string]$m.binding.failed_manifest_id-or
        $priorEvidence.manifest_sha256-ne[string]$m.binding.failed_manifest_sha256-or
    ...
    if($ev.status-ne'passed'-or[bool]$ev.cleanup_ambiguity-or-not[bool]$ev.namespace_reuse_blocked-or
        ...
        [int]$recoveryCounts.cred_write_w -ne 0 -or [int]$recoveryCounts.cred_read_w -gt [int]$m.limits.CredReadW -or
        [int]$recoveryCounts.cred_delete_w -gt [int]$m.limits.CredDeleteW -or [int]$recoveryCounts.cred_free -gt [int]$m.limits.CredFree -or
        [int]$recoveryCounts.total -ne $recoveryTrace.Count -or $recoveryTrace.Count -gt [int]$m.limits.total_native_calls -or
