...
public sealed record ProviderOperationAdmissionRequest(
...
public sealed record ProviderOperationAdmissionReceipt(
    DateTimeOffset EffectiveGateTime,
    DateTimeOffset DeadlineUtc);

public sealed record ProviderResponsePersistenceReceipt(
    string ResponseId,
    string UsageEntryId,
    string SettlementId,
    string ReplayEdgeId,
...
public sealed record ProviderReservationReadModel(
    long MaximumInputTokens,
    long MaximumOutputTokens,
    long ReservedNanoUsd,
    long CoordinatorFencingEpoch,
...
{
    private const long HistoricalProviderCommittedNanoUsd = 250_080_000;
    private const long ProviderHardBudgetNanoUsd = 10_000_000_000;

    public ProviderOperationAdmissionReceipt AdmitProviderOperation(
        ProviderOperationAdmissionRequest request)
    {
        ...
        { throw new InvalidOperationException("Provider operation admission is not an exact finite operation."); }
        lock (gate)
        {
            ...
            long currentCommitted = ScalarLong(
                ...
                """, transaction, ("$historical", HistoricalProviderCommittedNanoUsd));
            if (checked(currentCommitted + request.ReservedNanoUsd) > ProviderHardBudgetNanoUsd)
            { throw new InvalidOperationException("Provider operation admission would exceed the aggregate hard budget."); }
            Execute(
                """
                ...
                ("$campaign", request.OperationGroupId), ("$stage", request.Stage),
    ...
    public void RecordProviderOperationStart(string operationId, string attemptId,
        string requestId, string reservationId, string dispatchFenceId, DateTimeOffset occurredAt)
    {
        lock (gate)
        {
            ...
            if (exact != 1) { throw new InvalidOperationException("Provider operation possible start lacks exact live admission."); }
            Execute(
                """
                INSERT INTO m1_slice6_successor_v6_budget_events(
                  event_id,operation_id,reservation_id,event_kind,reserved_nano_usd,settled_nano_usd,
    ...
    public void ReleaseProviderOperationBeforeStart(string operationId, string reservationId,
    ...
    public ProviderResponsePersistenceReceipt RetainAmbiguousProviderOperation(
        ...
        lock (gate)
        {
            ...
            if (exactTerminal != 1 || terminalCount != 1)
            {
                throw new InvalidOperationException(
                    "Provider operation ambiguous recovery conflicts with existing terminal accounting.");
            }
            transaction.Commit();
    ...
    public ProviderResponsePersistenceReceipt PersistProviderResponse(
        ProviderSimulationPersistenceRequest request)
    {
        ArgumentNullException.ThrowIfNull(request);
        if (request.RawResponseBytes is null || request.RawResponseBytes.Length == 0)
        { throw new InvalidOperationException("Provider operation response persistence requires retained response bytes."); }
        lock (gate)
        {
            using SqliteTransaction transaction = BeginImmediateTransaction();
            ...
            if (exactRoot != 1) { throw new InvalidOperationException("Provider operation response lacks exact started admission."); }
            long reserved = ScalarLong("SELECT reserved_nano_usd FROM m1_slice6_successor_v6_operations "
            ...
            if (request.ResponseHeadersBytes is { Length: > 0 })
            {
                ...
                { throw new InvalidOperationException("Provider operation response headers exceed their retained bound."); }
            ...
            bool exactUsage = request.Usage.Availability == ProviderAvailabilityState.Available
                ...
                && HasExactProviderQuantity(request.Usage.DispatchCount)
                && HasExactProviderQuantity(request.Usage.InputTokens)
                && HasExactProviderQuantity(request.Usage.OutputTokens)
                && HasExactProviderQuantity(request.Usage.TotalTokens)
                && HasExactProviderQuantity(request.Usage.ReasoningTokens)
                && HasExactProviderQuantity(request.Usage.CacheReadTokens)
                && HasExactProviderQuantity(request.Usage.CacheWriteTokens)
                && HasExactProviderQuantity(request.Usage.PricedToolCalls)
                && HasExactProviderQuantity(request.Usage.CalculatedNanoUsd);
            long dispatch = ProviderQuantityValue(request.Usage.DispatchCount, exactUsage);
            long input = ProviderQuantityValue(request.Usage.InputTokens, exactUsage);
            long output = ProviderQuantityValue(request.Usage.OutputTokens, exactUsage);
            long total = ProviderQuantityValue(request.Usage.TotalTokens, exactUsage);
            long reasoning = ProviderQuantityValue(request.Usage.ReasoningTokens, exactUsage);
            long cacheRead = ProviderQuantityValue(request.Usage.CacheReadTokens, exactUsage);
            long cacheWrite = ProviderQuantityValue(request.Usage.CacheWriteTokens, exactUsage);
            long tools = ProviderQuantityValue(request.Usage.PricedToolCalls, exactUsage);
            long cost = ProviderQuantityValue(request.Usage.CalculatedNanoUsd, exactUsage);
    ...
    public ProviderReservationReadModel ReadProviderReservation(
        ...
        string reservationId, string dispatchFenceId)
    {
        lock (gate)
        {
            ...
            if (!reader.Read()) { throw new InvalidDataException("Provider operation recovery reservation is absent."); }
            ProviderReservationReadModel result = new(reader.GetInt64(0), reader.GetInt64(1),
                reader.GetInt64(2), reader.GetInt64(3), DateTimeOffset.Parse(reader.GetString(4),
                    System.Globalization.CultureInfo.InvariantCulture));
            if (reader.Read()) { throw new InvalidDataException("Provider operation recovery reservation is ambiguous."); }
            return result;
        }
    }

    private ProviderOperationReadModel ReadPersistedProviderOperation(string operationId)
    {
        using SqliteCommand command = connection.CreateCommand();
        command.CommandText =
            """
        ...
        { throw new KeyNotFoundException($"Provider operation '{operationId}' does not have terminal provider-operation evidence."); }
        ...
        if (reader.Read()) { throw new InvalidDataException("Provider operation provider operation is ambiguous."); }
        return result;
    }
    ...
    private (string Sha256, long Bytes) ReadPayloadIdentity(string payloadId, SqliteTransaction transaction)
    {
        ...
        using SqliteDataReader reader = command.ExecuteReader();
        if (!reader.Read()) { throw new InvalidOperationException("Provider operation retained payload identity is absent."); }
        (string, long) result = (reader.GetString(0), reader.GetInt64(1));
        if (reader.Read()) { throw new InvalidOperationException("Provider operation retained payload identity is ambiguous."); }
        return result;
    }

    private static bool HasExactProviderQuantity(ProviderQuantityContract quantity) =>
        quantity.Availability == ProviderAvailabilityState.Available && quantity.Value.HasValue;

    private static long ProviderQuantityValue(ProviderQuantityContract quantity, bool exact) =>
