...
public sealed class ProviderUsageAccounting
{
    private readonly AuthoritativeStore store;

    public ProviderUsageAccounting(AuthoritativeStore store)
    {
        this.store = store ?? throw new ArgumentNullException(nameof(store));
    }

    public void PublishExactCatalog(DateTimeOffset now) =>
        store.PublishProviderCatalog(OpenAiProviderProfileCatalog.Capability, OpenAiProviderProfileCatalog.Price, now);
        ...
        DateTimeOffset createdAt)
    {
        ...
        ProviderOperationKind operationKind = operation.OperationKind switch
        {
            ...
            _ => throw new InvalidDataException("The persisted provider operation kind is outside the closed provider profile."),
        };
