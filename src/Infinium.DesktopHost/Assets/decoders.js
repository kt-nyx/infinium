...
import { lifecycleStates, registeredMessages, } from "./generated/renderer-contract.generated.js";
import { validateRendererSchema } from "./schema-validator.js";
const capabilities = ["bootstrap", "run-query", "event-resync", "configuration", "provider-enrollment", "result-exploration", "durable-user-review"];
const availabilities = ["available", "partial", "unavailable"];
const scalarAvailabilities = ["available", "unavailable", "unsupported", "unknown"];
const resultKinds = ["supported-case", "lead-only-case", "finding", "abstention", "failure", "coverage-gap"];
const resultSorts = ["identity-ascending", "severity-descending-identity-ascending"];
const denominatorStates = ["known", "enumerating", "unavailable", "unknown", "unsupported"];
const eventKinds = ["progress", "lifecycle-changed", "projection-invalidated", "resync-required"];
const resyncReasons = ["slow-client", "queue-overflow", "sequence-gap", "coordinator-restart", "replay-window-expired", "projection-rebuilt", "cursor-invalid"];
const messageKinds = ["request", "response", "event"];
const catalogs = {
...
export function decodeClosedValue(key, value) {
    if (key === "outcome")
        return decodeOutcome(value);
    const catalog = catalogs[key] ?? [];
    if (typeof value !== "string" || !catalog.includes(value))
        throw new Error(`The renderer returned an unknown ${key} value.`);
    return value;
}
export function validateParsedRendererMessage(value) {
    ...
    if (!registeredMessages.some((entry) => entry.operation === operation && entry.messageKind === messageKind))
        throw new Error("The renderer operation/message pair is not registered.");
    ...
    if (messageKind !== "request") {
        ...
        if (outcome !== "accepted") {
            ...
            if (expectedCode === null) {
                if (["conflict", "unsupported", "unavailable", "cancelled", "indeterminate", "resync-required"].includes(code))
                    throw new Error("A rejected response carries a mismatched error code.");
            }
            else if (code !== expectedCode)
                throw new Error("A renderer failure carries a mismatched error code.");
            if (outcome === "conflict") {
                ...
                if (!["stale-revision", "already-applied", "resync-required"].includes(disposition))
                    throw new Error("The conflict disposition is unknown.");
            }
    ...
    if (operation === "results.list" && messageKind === "request") {
        ...
        if (payload.after_cursor !== undefined)
            assertOpaqueCursor(string(payload.after_cursor, "after_cursor"));
        for (const kind of array(payload.kinds, "kinds"))
            decodeClosedValue("resultKind", kind);
    ...
    if (operation === "results.list" && messageKind === "response" && payload.outcome === "accepted") {
        ...
        if (items.length > 100)
            throw new Error("The result page exceeds its item bound.");
        for (const item of items)
            validateResultSummary(item);
        const hasMore = boolean(page.has_more, "has_more");
        if (hasMore !== (page.next_cursor !== undefined))
            throw new Error("The result page cursor and has-more state are inconsistent.");
        if (page.next_cursor !== undefined)
            assertOpaqueCursor(string(page.next_cursor, "next_cursor"));
    }
    if (operation === "results.detail" && messageKind === "response" && payload.outcome === "accepted") {
        ...
        if (!/^[0-9a-f]{64}$/u.test(digest))
            throw new Error("The source payload fingerprint is invalid.");
    }
    if (operation === "progress.read" && payload.outcome === "accepted")
        validateProgressProjection(payload.progress);
    if (operation === "progress.subscribe" && messageKind === "event") {
        ...
        if (eventKind === "progress")
            ...
            if (recordKind !== "requested" && recordKind !== "observed")
                throw new Error("The lifecycle transition record kind is unknown.");
        }
        else {
...
function validateProgressProjection(value) {
    ...
    if (availability === "available")
        assertDecimalUInt64(string(total.value, "total_units.value"));
    for (const field of ["completed_units", "reused_units", "queued_units", "running_units", "failed_units", "skipped_units", "unsupported_units", "limited_units", "invalidated_units", "gap_units"])
        assertDecimalUInt64(string(summary[field], field));
    const cost = object(progress.cost, "cost summary");
    for (const field of ["reserved_nano_usd", "calculated_actual_nano_usd", "provider_input_tokens", "provider_output_tokens", "provider_reasoning_tokens", "provider_dispatch_count", "provider_tool_call_count"]) {
        ...
        if (scalarAvailability === "available") {
            ...
            if (field === "reserved_nano_usd" || field === "calculated_actual_nano_usd")
                assertDecimalInt64(decimal);
            else
                assertDecimalUInt64(decimal);
        }
    }
}
function validateResultSummary(value) {
    ...
    for (const field of ["item_id", "run_id", "logical_id"])
        assertOpaqueProductIdentity(string(item[field], field));
    if (item.case_occurrence_id !== undefined)
        assertOpaqueProductIdentity(string(item.case_occurrence_id, "case_occurrence_id"));
}
...
function object(value, label) {
    if (typeof value !== "object" || value === null || Array.isArray(value))
        throw new Error(`The renderer ${label} is not an object.`);
    return value;
}
function array(value, label) {
    if (!Array.isArray(value))
        throw new Error(`The renderer ${label} is not an array.`);
    return value;
}
function string(value, label) {
    if (typeof value !== "string")
        throw new Error(`The renderer ${label} is not a string.`);
    return value;
}
function boolean(value, label) {
    if (typeof value !== "boolean")
        throw new Error(`The renderer ${label} is not a boolean.`);
    return value;
}
