...
| RQ-004 | Which exact Mutagen.Bethesda package and authority boundary should Infinium accept, and which Skyrim SE plugin, archive, string, record, and field surfaces must be positively qualified or excluded? | Bethesda semantic layer ADR | Reopened for the Slice 4 compatibility disposition by RESEARCH-0053: accepted public RACE and REFR controls are not consumable through Mutagen `0.54.2`'s required semantic APIs. ADR-0009 remains authoritative, Slice 4 is blocked, and fixture correction/resealing, an explicit parser-boundary amendment, or another exact-version qualification requires owner disposition |
