...
| RQ-042 | How can Infinium capture the changed installation, expand an exact source finding or case to dependency-complete work, and execute a separately initiated successor through `managed-analysis-v1` while retaining exact lineage, honest coverage, and a closed renderer boundary? | Targeted-verification architecture and corrected WP6 plan | Investigated by RESEARCH-0058; Proposed ADR-0038 and the Proposed WP6 addendum await owner disposition, so current execution remains Unsupported |
