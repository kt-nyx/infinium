...
| RQ-004 | Which exact Mutagen.Bethesda package and authority boundary should Infinium accept, and which Skyrim SE plugin, archive, string, record, and field surfaces must be positively qualified or excluded? | Bethesda semantic layer ADR | Resolved again for M1 by the owner-selected Option A disposition of RESEARCH-0053: erroneous Bethesda fixture version `1.0.0` was superseded by independently resealed `1.0.1`; the corrected RACE and canonical CELL-contained REFR controls pass the retained Mutagen `0.54.2` conformance probes, so ADR-0009 remains unchanged. Slice 4 production implementation still requires a fresh attempt and its full EVAL-0052/EVAL-0086 gates |
