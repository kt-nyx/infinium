# RESEARCH-0024: Wave C analysis, taxonomy, corpus, and scale integration

Status: Accepted  
Date: 2026-07-25  
Last reviewed: 2026-07-26  
Researcher: Codex agent  
Owner disposition: All Wave C research recommendations accepted on 2026-07-25

## 1. Purpose

This report records the integrated owner disposition for RESEARCH-0014 through
RESEARCH-0023. It distinguishes acceptance of a research recommendation from
an executed analyzer-support, corpus, performance, or evaluation claim.

## 2. Accepted recommendations

| RQ | Accepted Wave C disposition |
|---|---|
| RQ-019 | Use static structural inventory, layered physical/effective identity, and versioned relationship manifests for root/native components; named analyzer breadth remains conditional |
| RQ-020 | Begin with generic generated-output inspection and add version-pinned, read-only artifact adapters; do not infer freshness from artifact presence alone |
| RQ-021 | Build a generic exact-byte/provider/syntax layer, then prioritize MCM Helper, separate SPID/KID/BOS schemas, and OAR structural indexing |
| RQ-022 | Bound compiled Papyrus analysis to qualified static PEX declarations, instructions, VMAD relationships, and explicit gaps; do not make generic behavior or performance claims |
| RQ-023 | Use NIF typed-reference completeness as the first asset slice and preserve provider/reference provenance; qualify FaceGen identity/provider behavior before relying on it |
| RQ-024 | Build a generic semantic substrate, prove narrow NPC scope-incongruent reversion, generalize through REFR placement/activation/enable/linked-reference topology and a narrow forced-alias relationship, then expand by risk and qualified semantics |
| RQ-025 | Use synthetic-first fixtures plus private, hash-pinned controlled-real candidates; never make the creator's profile or a real mod identity a production rule |
| RQ-027 | Use the accepted scale, progress, resource, and provider-neutral cost measurement contract; treat current observations as rough feasibility only and remeasure exact budgets against authoritative adapters and candidate architectures |
| RQ-035 | Use snapshot-bound typed indexes, causal joins, canonical participants, explicit negative/gap handling, and score-independent mandatory lanes instead of naïve all-pairs model comparison |
| RQ-036 | Adopt `infinium.skyrim-se.mod-impact-taxonomy/0.1.0` as the normative Skyrim SE product taxonomy |

The EVAL-0032 candidate-selection and EVAL-0086 taxonomy-classification
specifications are accepted for planning. Neither has passed execution.

## 3. Accepted taxonomy

The normative product result is
[Skyrim SE mod-impact taxonomy version 0.1.0](../../product/mod-impact-taxonomy.md).
It has five independent axes:

1. declared purpose and intended target;
2. technical modification surface;
3. affected game system or content area;
4. consequence type; and
5. faceted effect extent.

It is versioned, hierarchical where useful, multi-label, and open to
evidence-backed extension. Unknown, unsupported, unmapped, and not-applicable
are distinct applicability states. Severity, confidence, symptoms, evidence
authority, analyzer maturity, readiness, review state, and installation
topology remain separate.

The taxonomy dependency map and its product, architecture, and evaluation
consumers were updated during acceptance. Acceptance does not imply complete
taxonomy coverage or analyzer support.

## 4. Gate C assessment

Wave C investigation and owner-decision work is complete, but Gate C is not yet
fully met.

Satisfied portions:

- the product owner accepted taxonomy version `0.1.0` as a product
  specification rather than an ADR;
- affected product, architecture, and evaluation documents use the accepted
  taxonomy;
- the first semantic proof remains generic scope-incongruent reversion rather
  than an NPC-specific production rule;
- the synthetic candidate experiment includes matched negatives, materially
  different surfaces, explicit gaps, canonical participants, and no dense
  all-pairs model comparison;
- EVAL-0032 and EVAL-0086 have accepted specifications.

Remaining Gate C prerequisites:

1. execute and independently review the loose-only FaceGen full/light,
   applicability, origin-identity, and provider matrix required by RQ-023; and
2. finish a reproducible, independently grounded EVAL-0016 manifest and select
   and qualify a materially different EVAL-0017 real-mod case under RQ-025.

AI Overhaul plus Children of the Pariah remains an incomplete EVAL-0016
candidate. The surveyed Ryn placement combination remains a discovery lead,
not EVAL-0017. Approval of RESEARCH-0020 explicitly preserves those limits.

## 5. Performance and evaluation limits

RESEARCH-0022's retained artifacts support the accepted logical
candidate-selection direction for their author-coupled synthetic scope. They
do not pass EVAL-0032 or establish real-mod recall, finding accuracy, causal
case grouping, persistence behavior, or production capacity.

RESEARCH-0023 establishes a measurement method and rough local feasibility
bounds only. Its RQ-027-specific measurement history is not a durable,
replayable production benchmark. Exact scan-time, memory, IO, disk, checkpoint,
and LLM-cost budgets must be measured after authoritative adapters and
candidate architecture prototypes exist.

## 6. Downstream effect

Wave D may proceed using:

- the accepted taxonomy for declared-purpose and claimed-area extraction;
- the accepted evidence-authority boundary;
- the accepted source-retention and policy constraints;
- the accepted installed-mod identity model; and
- explicit Wave C unsupported and unevaluated regions.

Wave E may compare architectures against the accepted candidate-selection,
progress, checkpoint, resource, cost, and taxonomy persistence contracts. It
must not select an architecture from RESEARCH-0023's preliminary figures
alone.

M0 still requires Gate C closeout, Waves D and E, and Wave F integration and M1
planning before production implementation begins.
