...
| Microsoft.ML.Tokenizers and OpenAI tiktoken primary sources | **Accepted repository-local WP1 implementation evidence under ADR-0023 and the Slice 6 plan** | Retrieved 2026-08-11 | Exact offline `Microsoft.ML.Tokenizers`/`O200kBase` 2.0.0 selection, MIT licensing, o200k mapping/evidence, patched transitive dependency, and conservative byte-envelope limitations recorded by [RESEARCH-0055](investigations/RESEARCH-0055-slice6-local-input-bound-policy.md); this grants no provider or credential authority |
