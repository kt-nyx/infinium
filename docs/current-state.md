...
| Next eligible action | Refreeze the exact credential-recovery boundary assertion, bind its exact clean four-document campaign successor, then rerun the complete non-live floor and fresh independent review. Credential execution remains ineligible. |
...
is excluded by NonLiveAll; current authority is correction and non-live
reverification only.
...
Current authority is correction and non-live reverification only. The
replacement retains a separately committed zero-effect credential handoff and distinct committed
