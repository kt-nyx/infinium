...
| Next eligible action | Freeze and bind the exact campaign Layer 6 path-set correction, prove repeated explicit and ordinary builds reproduce the complete closure, then rerun the complete non-live floor and fresh independent review. Credential execution remains ineligible. |
...
unknown replacement bytes. B8 is excluded because its exact campaign Layer 6
path set omitted `Directory.Build.targets`; current authority is correction
and non-live reverification only.
