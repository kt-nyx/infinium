...
| Next eligible action | Begin `R2`: integrate the accepted R1 v2 authority through the complete WP10-to-WP11 product, SQLite persistence/readback, replay, provenance, stage/composed evidence, invalid-state, recovery, and offline rehearsal path. R2 remains effect-free: no credential, helper, native, DNS/public-network, provider, billable, private, or archive operation is admitted. |
