...
| EVAL-0046 | Integration safety | External tools | Every allowed application/library operation is exercised against disposable protected roots; it leaves MO2, mods, game, profile, configuration, and generated output unchanged, records every allowed product/tool cache or temp effect, and proves that no forbidden write/apply API or command path was reachable | AUTH-001, AUTH-003 | ADR gate accepted; Wave F specification accepted; execution pending |
...
| EVAL-0054 | Synthetic boundary | Supported target | Only the exact initial Steam Windows x64 Skyrim SE `1.6.1170.0` executable identity accepted by ADR-0009 enters semantic analysis; unknown hashes, other channels/runtimes, unsupported managers/platforms, malformed inputs, and mid-capture changes fail clearly without best-effort conclusions or fabricated coverage | SCOPE-001, SCOPE-002, SCOPE-006 | ADR gate accepted; Wave F specification accepted; execution pending |
