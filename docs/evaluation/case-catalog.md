...
| EVAL-0033 | Synthetic adversarial | Untrusted content | Prompt/HTML/search/tool-output instructions cannot grant source, local-state, or operation authority; cannot obtain internal tools or secrets; and cannot alter analysis/source policy | SEC-001 | Phase D exercises the desktop facet with inert hostile result text, restrictive CSP/Trusted Types, a closed generated operation registry, no host objects or generic native proxy, and value-free browser-policy refusal text; broader Wave F analysis-content execution remains pending |
| EVAL-0034 | Synthetic adversarial | Credentials/context minimization | Secrets and unnecessary usernames, absolute paths, and unrelated values are absent from prompts, logs, traces, and exports while required non-secret context remains usable; confirmed credential deletion prevents queued/new/retry or reserved-but-undispatched use, releases unused budget reservation, and leaves uncancellable in-flight work disclosed | SEC-002, SEC-004, AI-003, AI-004 | Phase D exercises the desktop facet with a secret canary absent from renderer state, WPF notice text, serialized bridge traffic, ordinary coordinator output, exclusive WebView failure/reload artifacts, and inspected files; credential lifecycle remains covered by its owning package and broader Wave F execution remains pending |
| EVAL-0035 | Synthetic adversarial | Privileged boundary | Out-of-scope path, URL, command, and tool arguments are rejected | AUTH-002, SEC-003 | Phase D exercises the desktop facet with exact-origin/message validation, live denied navigation/frame/resource/window/download/permission attempts, eight-variable and seven-policy-family inherited WebView2 override denial before creation/recovery, opaque qualification identity, and no renderer path/URL/command/provider authority; broader Wave F execution remains pending |
...
| EVAL-0064 | Contract/integration | Offline/provider boundary | A local-only run requires no provider credentials; unavailable network/LLM capabilities are explicit; OpenAI may be the sole initial provider; later adapters declare their own capabilities without altering provider-independent domain/evidence contracts or being required to emulate OpenAI search | AI-001, AI-002, OPS-001 | Passed for the bounded M1 offline-by-default local path and explicit OpenAI development-provider boundary; Phase D additionally proves the packaged diagnostic bootstrap/results/detail/progress/events/cancel path locally with no credential and no remote renderer dependency; additional providers and shipped-product enrollment remain later work |
...
| EVAL-0088 | Synthetic/integration boundary | Process, IPC, and query contract | Coordinator-start races select one fenced authority; client/worker/helper roles, protocol versions, nonces, message and page limits, cursors, streams, cancellation semantics, reconnect/resync, worker staging, and coordinator-only publication fail closed and remain correct across malformed input, slow clients, and process crashes | SCAN-004, SCAN-005, AUTH-002, SEC-003 | The bounded M1 coordinator, worker/helper protocol, fencing, malformed input, reconnect, staging, and crash-recovery obligations passed. Phase D additionally exercises exact renderer origin/version/session/directional-sequence/gesture checks, 1 MiB messages, 64-item stream/backpressure behavior, transport cancellation, authoritative resync, same-session coordinator reconnect, and renderer/browser/shell recovery; no broader operational mode is claimed. |
| EVAL-0089 | Synthetic/integration failure | Credential lifecycle and recovery | Exact-target OS-backed enrollment, verification, replacement, disable, deletion, restart recovery, unavailable-store handling, size limits, metadata/secret half-commit recovery, backup/restore reauthentication, and dispatch races preserve generation/revocation authority without secret leakage, fallback, or continued undispatched use | SEC-002, SEC-004, AI-004, AI-007 | The bounded M1 credential enrollment/verification/rotation/disable/deletion, recovery, unavailable-store, size-limit, race, and secret-canary obligations passed. Phase D adds only proof that the local diagnostic requests no credential and that its secret canary is absent from renderer state, WPF, bridge traffic, ordinary IPC, logs, and failure/reload artifacts; it does not claim an additional credential-lifecycle or enrollment workflow. |
| EVAL-0090 | Contract/integration | Frontend application bootstrap and live contract | Generated clients negotiate versions; obtain bounded health/capability/configuration/recent-work bootstrap state; use typed query/command/error/conflict/cancellation states; and recover through authoritative snapshot/resync after stale cursor, event loss, renderer/shell restart, or coordinator restart without UI-owned durable truth | SEC-003, SCAN-004, SCAN-005, OPS-005 | Checkpoint D is accepted. The WP9 integrated candidate reruns generated version/fingerprint negotiation and real bootstrap, query, progress, event, host-attested cancellation, authoritative resync/resubscribe, three renderer reloads, renderer/browser failure recreation, coordinator termination/restart, and shell restart. Separate request/event sequence domains and stale-session tests preserve coordinator-owned truth. Awaiting Checkpoint E acceptance. |
| EVAL-0091 | Contract/integration safety | Setup, profile, configuration, estimate, and enrollment | Exact MO2/applicable LOOT states and capability gaps are visible; tool override is typed and validated; saved MO2 selection remains suggestion-only; one profile is explicitly confirmed; saved scan configurations preserve revisions and immutable effective run values; pre-run estimates expose authority gaps; and provider enrollment returns only non-secret outcomes | SCOPE-003, SCOPE-004, SCAN-001, SCAN-002, SCAN-003, SCAN-009, TOOL-001, TOOL-002, TOOL-003, SEC-002 | The WP9 integrated candidate reruns the accepted Phase B typed setup/profile/configuration/estimate/non-secret status path through the generated native consumer and proves immutable effective run values across reconnect/restart. Those RPCs remain deliberately unmapped from the diagnostic renderer; six provider RPCs remain declared-unimplemented; no credential, live provider, fallback, or polished M2 setup workflow is claimed. Awaiting Checkpoint E acceptance. |
| EVAL-0092 | Contract/integration/scale | Frontend result exploration | Summary/readiness or scope-limited status, supported-case and lead-only queues, finding/case/report details, evidence/provenance expansion, and focused-mod views preserve canonical truth, uncertainty, coverage, and gaps through bounded server-side filters/sorts/search/keyset pages; 100,000 synthetic summaries are virtualized without full-population transfer | PROD-003, PROD-004, UX-001, UX-002, UX-003, UX-004, OPS-005 | The WP9 integrated candidate composes accepted FindingReport/readback and native result exploration with the accepted real diagnostic bridge over 150 hostile-text findings, bounded 100-item keyset pages/detail, and deterministic 100,000-summary/1,000-page virtualization with at most 500 cached summaries and 13 mounted rows. Complete result presentation remains implementation-active until consumed by the real M2 interface. Awaiting Checkpoint E acceptance. |
| EVAL-0093 | Contract/integration failure | Review state, assumptions, targeted verification, and export | Revision-bound append-only dispositions, suppression, annotations, and assumptions survive concurrency/restart without rewriting analysis; carryover obeys causal/revision proof; targeted verification creates separately initiated scope-linked work without borrowing unrelated readiness; and local-private structured export records exact selection/provenance/privacy/source-policy decisions without mutating source objects | INTENT-004, INTENT-005, FIND-005, FIND-006, FIND-007, FIND-014, UX-005, OPS-002, OPS-003 | The WP9 integrated candidate reruns append-only review/assumption continuity, export creation/inspection/deletion and source immutability, recursive/tamper rejection, and generated native targeted-verification begin/status/cancel/start/readback with bounded preparation, dependency/correlation closure, atomic successor admission, immutable lineage, recovery, and migration. The five targeted-verification RPCs remain native-only and unmapped. Real M2 interaction remains implementation-active. Awaiting Checkpoint E acceptance. |
| EVAL-0094 | Integration/adversarial/accessibility/performance | Desktop host and renderer bridge qualification | Packaged React runs under the controlled WebView2 origin; the generated closed bridge rejects wrong-origin/version/session/sequence/gesture, malformed, oversized, replayed, out-of-order, arbitrary path/command/URL/provider/credential, navigation/download/permission/new-window, DevTools, and remote-debug attempts; real paginated query/progress/cancellation/reconnect paths work; missing/outdated runtime and renderer failure recover; representative keyboard/focus/naming/landmark/contrast/zoom/reduced-motion/screen-reader checks pass; and startup/memory/query/bridge/package/runtime/license measurements are retained | SCOPE-006, SEC-001, SEC-002, SEC-003, SEC-004, UX-006, OPS-005 | The WP9 repeat passes the protected exact-origin WPF/WebView2/React host, stable-only runtime selection, inherited-override refusal/revalidation, hostile bridge/policy/runtime/lifecycle cases, real populated operations, accessibility mechanics, six fresh launches, and zero process survivors. Fresh measurements are planning observations, not production guarantees; Chromium AX and Windows UI Automation remain the screen-reader evidence and no manual Narrator walkthrough is claimed. Awaiting Checkpoint E acceptance. |
