# M1 Slice 5 WP5 product-blind operational fixtures v1.0.2

Status: `independently reviewed; comparison complete 12/12; explicit native capability gaps`

Registry: `infinium.m1s5.wp5.operational-cases.20260809.3`

Independent review:
[`codex-product-blind-wp5-v1.0.2-review-20260809`](independent-review.md),
...
- Development:
  `infinium.m1s5.wp5.publication-replay-query-output-recovery-safety.lantern-a/1.0.2`
- Validation:
  `infinium.m1s5.wp5.publication-replay-query-output-recovery-safety.compass-b/1.0.2`

| Family | Development | Materially independent validation |
|---|---|---|
| atomic publication | `WP5-LANTERN-PUBLICATION-D01` — payload/edge staging fault | `WP5-COMPASS-PUBLICATION-V01` — durable-intent/pre-commit fault with different graph and revision |
| replay/invalidation | `WP5-LANTERN-REPLAY-D02` — clean/incremental/replay equality and one graph change | `WP5-COMPASS-REPLAY-V02` — same-identity changed-fact drift plus equal-fact different-identity substitute |
| bounded query | `WP5-LANTERN-QUERY-D03` — rank-descending, identity-ascending, page size 2 | `WP5-COMPASS-QUERY-V03` — time-descending, identity-descending, page size 3 |
| terminal/equivalent output | `WP5-LANTERN-OUTPUT-D04` — four terminal states with one fact/gap population | `WP5-COMPASS-OUTPUT-V04` — four states with different fact, gap, and review populations |
| recovery | `WP5-LANTERN-RECOVERY-D05` — crash after durable stage | `WP5-COMPASS-RECOVERY-V05` — crash after checkpoint and before stage |
| write/non-mutation safety | `WP5-LANTERN-SAFETY-D06` — delta topology, ordinal order, mixed authorized reparse/final-handle controls | `WP5-COMPASS-SAFETY-V06` — epsilon topology, permuted order, independently reassigned reparse/root/race outcomes |
...
Atomic publication, recovery, and cursor cases remain narrower WP5 slice
