...
complete required M1 case set on one final candidate, and produce the M1
completion record.

The accepted plan permits no in-place product-contract revision, no
storage-schema revision beyond frozen storage 1.10.0/schema 11, and no new
product semantics. If implementation evidence shows that the frozen contracts
cannot express the required composition truthfully, that is a genuine
architecture/owner stop; the implementer must not invent a successor contract
or reinterpret a frozen field.

## Predecessor and next gate
