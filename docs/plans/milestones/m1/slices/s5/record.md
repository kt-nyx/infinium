...
## Repository-normalization interpretation

The accepted [pre-closeout normalization amendment](repository-normalization.md)
supersedes the prior terminal candidate only as the exact closeout candidate.
The functional old-to-new path, identity, tool, verification, and phase map is
recorded in the [accepted Slice 5 plan](plan.md#repository-normalization-mapping).
Historical commands, package IDs, Git paths, artifact paths, review reports,
and source-authority seals below intentionally retain the names that were true
when their evidence was produced. They are chronology, not current navigation.
...
[development execution policy](../../../../../execution-policy.md) for
...
## Pre-closeout repository normalization

On 2026-08-10 the project owner accepted the
[repository-normalization amendment](repository-normalization.md) before Slice
5 closeout. The documentation portion:

- moved the repository execution policy to `docs/execution-policy.md`;
- reorganized plans as milestone -> slice with plans, amendments, records, and
  stable reviews beside their owning scope;
...
- retained three small historical redirects required by the immutable `/4`
  authority matrix while keeping their original bytes Git-only.

Docs-focused validation parsed 14 tracked documentation JSON files strictly,
resolved 163 Markdown files with zero missing local targets, found zero
