...
public sealed class AnalysisStatePersistenceTests
{
    ...
    public void AnalysisStateModelSchema8HasExactMigrationContractAndObjects()
    {
        ...
        Assert.AreEqual("8", ScalarText(connection, "PRAGMA user_version;"));
        Assert.AreEqual(
            "8",
            ScalarText(
                connection,
        ...
        Assert.AreEqual(
            ...
            ScalarText(
                ...
                SELECT CAST(from_version AS TEXT) || '|' || CAST(to_version AS TEXT)
                FROM migration_history
                WHERE migration_id = 'M1-S6-SUCCESSOR-0007';
                """));
        Assert.AreEqual(
            "7|8",
            ScalarText(connection,
                "SELECT CAST(from_version AS TEXT) || '|' || CAST(to_version AS TEXT) "
                + "FROM migration_history WHERE migration_id='M1-S6-SUCCESSOR-V6-0008';"));
    ...
    public void AnalysisStateModelMigratesAcceptedSchema5ForwardAndRefusesNewerSchema()
    {
        ...
        using (SqliteCommand command = connection.CreateCommand())
        {
            command.CommandText =
                ...
                "DROP TABLE m1_slice6_successor_v6_responses;" +
                "DROP TABLE m1_slice6_successor_v6_budget_events;" +
                "DROP TABLE m1_slice6_successor_v6_operations;" +
        ...
        {
            Assert.AreEqual(8, migrated.GetSchemaVersion());
        }
