...
public sealed class CrossStageCorpusIntegrationTests
{
    ...
    public void FrozenCrossStageComprehensiveCorpusExecutesDocumentationThroughOperationalBeforeOracleComparison()
    {
        ...
        string root = Path.Combine(Path.GetTempPath(), $"infinium-cross_stage-corpus-{Guid.NewGuid():N}");
        ...
        try
        {
            ...
            CoordinatorAuthority authority = store.AcquireCoordinatorAuthority(
                "cross_stage-comprehensive-corpus", DateTimeOffset.UtcNow, TimeSpan.FromMinutes(10));
            RunBinding binding = new("snapshot.001", "context.001", "configuration.001", "manifest.001");
            const string runId = "run-cross_stage-comprehensive-clean";
            ...
            AnalysisV1WorkAssignment assignment = new(
                ...
                "assignment-cross_stage-comprehensive-clean",
                ...
                "cross-stage corpus comprehensive clean corpus completed",
            ...
            JsonElement oracle = expected.RootElement.GetProperty("cases").EnumerateArray()
                .Single(item => item.GetProperty("case_id").GetString() == "CROSS-STAGE-CLEAN-D01")
        ...
        string runId, RunBinding binding, JsonElement revision, JsonElement shared)
    {
        ...
        DocumentationClaimInputContract[] claims = revision.GetProperty("claims").EnumerateArray()
            .Select(claim => new DocumentationClaimInputContract(
                ...
                claim.GetProperty("kind").GetString() switch
                {
                    ...
                    _ => throw new InvalidDataException("The cross-stage corpus claim kind is outside the closed product mapping."),
                },
        ...
        return new(Id(runId), Id(runId), DocumentationImportMode.CleanImport,
            Id("dependency.source.001"), Id("extractor.cross_stage"), new UtcTimestamp(DateTimeOffset.UnixEpoch),
        ...
        string runId, RunBinding binding, JsonElement shared, DocumentationEvidenceContract documentation)
    {
        ...
        AnalysisExecutionInputContract execution = new(
            ContractConstants.AnalysisExecutionInputSchemaId, Version(), Id("execution.cross_stage.clean"), Id(runId),
            Reference(binding.InstallationSnapshotId), Reference("bethesda.cross_stage"),
    ...
    private static FindingCaseInputContract FindingInput(CandidateAnalysisContract candidates, JsonElement shared)
    {
        ...
        TaxonomyClassificationFactContract[] taxonomy = candidates.Hypotheses.Select(hypothesis =>
            new TaxonomyClassificationFactContract(
                ...
                hypothesis.SupportingEvidenceIds, [Id("taxonomy-condition.cross_stage")], null,
                Id("analyzer.001"), new UtcTimestamp(DateTimeOffset.UnixEpoch),
                "Generic synthetic cross-stage corpus classification fact.")).ToArray();
    ...
    private static ClaimApplicabilityState Applicability(string value) => value switch
    {
        ...
        _ => throw new InvalidDataException("The cross-stage corpus applicability value is outside the closed product mapping."),
    };
    ...
    private static string LifecycleStateToken(LifecycleState value) => value switch
    {
        ...
        _ => throw new InvalidDataException("The cross-stage corpus product lifecycle is not terminal."),
    };
