...
public sealed class ManagedCrossStageCorpusIntegrationTests
{
    ...
    public async Task FrozenCrossStageFourCaseCorpusExecutesManagedCoordinatorAndTypedQueryBeforeOracleComparison()
    {
        ...
        string root = Path.Combine(Path.GetTempPath(), $"infinium-cross_stage-managed-corpus-{Guid.NewGuid():N}");
        ...
        try
        {
            ...
            CoordinatorAuthority authority = store.AcquireCoordinatorAuthority(
                "cross_stage-managed-corpus", DateTimeOffset.UtcNow, TimeSpan.FromMinutes(10));
            ...
            foreach (JsonElement expectedCase in expected.RootElement.GetProperty("cases").EnumerateArray())
            {
                ...
                AssertCase(expectedCase.GetProperty("expected"), observations[caseId], observations["CROSS-STAGE-CLEAN-D01"]);
            }
    ...
    public void ManagedRequestRejectsDeliveredInputFingerprintOrSourceReferenceDriftBeforeAdmission()
    {
        ...
        ManagedAnalysisOrchestrationRequest request = ManagedRequest(
            "run-cross_stage-drift", binding, bethesda, shared, requestInput, null, null);
        ...
        DocumentationEvidenceContract? retainedDocumentation)
    {
        ...
        ReplayMode mode = requestInput.GetProperty("mode").GetString() switch
        {
            ...
            _ => throw new InvalidDataException("cross-stage corpus request mode is outside the closed mapping."),
        };
        ...
        string runId, RunBinding binding, JsonElement revision, JsonElement shared)
    {
        ...
        DocumentationClaimInputContract[] claims = revision.GetProperty("claims").EnumerateArray().Select(claim =>
            new DocumentationClaimInputContract(
                ...
                claim.GetProperty("kind").GetString() switch
                {
                    ...
                    _ => throw new InvalidDataException("cross-stage corpus claim kind is outside the closed mapping."),
                },
        ...
        return new(
            ...
            Id("extractor.cross_stage"), new UtcTimestamp(DateTimeOffset.UnixEpoch), manifest, bytes, null, [target]);
    }
        ...
        DeliveredIndexCandidatePopulationSource source)
    {
        ...
        TaxonomyClassificationFactContract[] taxonomy = withHypothesis.Select(item =>
            new TaxonomyClassificationFactContract(
                ...
                [Id("taxonomy-condition.cross_stage")], null, source.AnalyzerId,
                new UtcTimestamp(DateTimeOffset.UnixEpoch), "Generic synthetic cross-stage corpus classification fact.")).ToArray();
        ...
        return new(
            Id("promotion.cross_stage"), Version(), Id("reconciliation.cross_stage"), Version(), Id("actor.cross_stage"),
    ...
    private static ProjectedCandidate Project(CandidatePipelineRequest request, CausalJoinPopulationMember member)
    {
        CandidateDecisionDisposition disposition = member.InputState switch
        {
            ...
            _ => throw new InvalidDataException("cross-stage corpus delivered fact has an unexpected decision state."),
        };
    ...
    private static void AssertCase(JsonElement expected, ManagedCaseObservation actual, ManagedCaseObservation clean)
    {
        ...
        switch (actual.CaseId)
        {
            case "CROSS-STAGE-CLEAN-D01":
                ...
                break;
            case "CROSS-STAGE-UNCHANGED-D02":
            ...
            case "CROSS-STAGE-REPLAY-D04":
                ...
                break;
            default:
                Assert.Fail("Unexpected cross-stage corpus case identity: " + actual.CaseId);
                break;
        }
    }
    ...
    {
        string directory = Environment.GetEnvironmentVariable("INFINIUM_CROSS_STAGE_RECEIPT_ROOT")
            ?? Path.Combine(repositoryRoot, "artifacts", "analysis-pipeline", "cross_stage");
        Directory.CreateDirectory(directory);
        byte[] bytes = JsonSerializer.SerializeToUtf8Bytes(new
        {
            schema_id = "infinium.verification.analysis-pipeline.cross_stage-product-comparison/v1",
            result = "passed",
    ...
    {
        RunRecord producer = store.CreateRun("command-cross_stage-bethesda", "run-cross_stage-bethesda", binding,
            authority.FencingEpoch, DateTimeOffset.UtcNow);
        _ = store.Transition("transition-cross_stage-bethesda", producer.RunId, producer.Generation,
        ...
        _ = store.Transition("terminal-cross_stage-bethesda", producer.RunId, current.Generation,
    ...
    private static ClaimApplicabilityState Applicability(string value) => value switch
    {
        ...
        _ => throw new InvalidDataException("cross-stage corpus applicability is outside the closed mapping."),
    };
