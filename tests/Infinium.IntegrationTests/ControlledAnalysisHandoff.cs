...
internal static class ControlledAnalysisHandoff
{
    ...
    internal static AnalysisCompositionEnvelope LoadControlledComposition()
    {
        string declared = Environment.GetEnvironmentVariable("INFINIUM_CONTROLLED_ANALYSIS_RETAINED_OUTPUT_ROOT") ?? DefaultRoot;
        ...
        {
            throw new InvalidDataException("The controlled scope-reversion retained-output root failed containment and non-reparse admission.");
        }
        ...
        {
            throw new InvalidDataException("The exact accepted controlled scope-reversion identity receipts drifted before retained-result access.");
        }
        ...
        if (receipt.RootElement.GetProperty("status").GetString() != "passed"
            || receipt.RootElement.GetProperty("handoff_id").GetString() != AcceptedAnalysisCompositionFixtures.ControlledHandoffId
            || receipt.RootElement.GetProperty("input_manifest_sha256").GetString()
                != AcceptedAnalysisCompositionFixtures.ControlledManifestSha256
        ...
        {
            throw new InvalidDataException("The controlled scope-reversion receipt content failed exact activation identity admission.");
        }
        ...
        try
        {
            ...
            AnalysisCompositionEnvelope envelope = AcceptedAnalysisCompositionFixtures.CreateControlled(
                payloads,
                [
            ...
            {
                throw new InvalidDataException("The accepted controlled scope-reversion retained-output root changed during read-only composition.");
            }
            return envelope;
        }
        finally
    ...
    private static string ContainedFile(string root, string name)
    {
        ...
        {
            throw new InvalidDataException("A controlled scope-reversion identity receipt escaped or reparsed its accepted root.");
        }
        return path;
    }
