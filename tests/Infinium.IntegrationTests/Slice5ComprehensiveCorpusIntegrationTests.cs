...
[TestClass]
public sealed class Slice5ComprehensiveCorpusIntegrationTests
{
    ...
    public void FrozenWp6ComprehensiveCorpusExecutesWp2ThroughWp5BeforeOracleComparison()
    {
        string fixtureRoot = Path.Combine(FindRepositoryRoot(), "docs", "evaluation", "fixtures", FixtureDirectory);
        ...
        string root = Path.Combine(Path.GetTempPath(), $"infinium-wp6-corpus-{Guid.NewGuid():N}");
        ...
        try
        {
            ...
            CoordinatorAuthority authority = store.AcquireCoordinatorAuthority(
                "wp6-comprehensive-corpus", DateTimeOffset.UtcNow, TimeSpan.FromMinutes(10));
            RunBinding binding = new("snapshot.001", "context.001", "configuration.001", "manifest.001");
            const string runId = "run-wp6-comprehensive-clean";
            RunRecord queued = store.CreateRun("command-wp6-comprehensive-clean", runId, binding,
                authority.FencingEpoch, DateTimeOffset.UtcNow);
            _ = store.Transition("transition-wp6-comprehensive-clean", runId, queued.Generation,
                LifecycleState.Running, authority.FencingEpoch, "execute frozen WP6 corpus", DateTimeOffset.UtcNow);
            ...
            AnalysisV1WorkAssignment assignment = new(
                ...
                "assignment-wp6-comprehensive-clean",
                ...
                "WP6 comprehensive clean corpus completed",
            ...
            JsonElement oracle = expected.RootElement.GetProperty("cases").EnumerateArray()
                .Single(item => item.GetProperty("case_id").GetString() == "WP6-CROSS-CLEAN-D01")
        ...
        string runId, RunBinding binding, JsonElement revision, JsonElement shared)
    {
        ...
        DocumentationClaimInputContract[] claims = revision.GetProperty("claims").EnumerateArray()
            .Select(claim => new DocumentationClaimInputContract(
                ...
                claim.GetProperty("kind").GetString() switch
                {
                    ...
                    _ => throw new InvalidDataException("The WP6 claim kind is outside the closed product mapping."),
                },
        ...
        return new(Id(runId), Id(runId), DocumentationImportMode.CleanImport,
            Id("dependency.source.001"), Id("extractor.wp6"), new UtcTimestamp(DateTimeOffset.UnixEpoch),
        ...
        string runId, RunBinding binding, JsonElement shared, DocumentationEvidenceContract documentation)
    {
        ...
        AnalysisExecutionInputContract execution = new(
            ContractConstants.AnalysisExecutionInputSchemaId, Version(), Id("execution.wp6.clean"), Id(runId),
            Reference(binding.InstallationSnapshotId), Reference("bethesda.wp6"),
    ...
    private static FindingCaseInputContract FindingInput(CandidateAnalysisContract candidates, JsonElement shared)
    {
        ...
        TaxonomyClassificationFactContract[] taxonomy = candidates.Hypotheses.Select(hypothesis =>
            new TaxonomyClassificationFactContract(
                ...
                hypothesis.SupportingEvidenceIds, [Id("taxonomy-condition.wp6")], null,
                Id("analyzer.001"), new UtcTimestamp(DateTimeOffset.UnixEpoch),
                "Generic synthetic WP6 classification fact.")).ToArray();
    ...
    {
        JsonElement wp2 = oracle.GetProperty("wp2");
        Assert.AreEqual(wp2.GetProperty("revisions").GetInt32(), actual.Revisions);
        Assert.AreEqual(wp2.GetProperty("imports").GetInt32(), actual.Imports);
        Assert.AreEqual(wp2.GetProperty("passages").GetInt32(), actual.Passages);
        Assert.AreEqual(wp2.GetProperty("claims").GetInt32(), actual.Claims);
        Assert.AreEqual(wp2.GetProperty("applications").GetInt32(), actual.Applications);
        Assert.AreEqual(wp2.GetProperty("purpose_assignments").GetInt32(), actual.PurposeAssignments);
        Assert.AreEqual(wp2.GetProperty("contradiction_gaps").GetInt32(), actual.DocumentationGaps);
        Assert.AreEqual(wp2.GetProperty("failures").GetInt32(), actual.DocumentationFailures);
        JsonElement decisions = oracle.GetProperty("wp3").GetProperty("decisions");
        ...
        Assert.AreEqual(oracle.GetProperty("wp3").GetProperty("candidates").GetInt32(), actual.Candidates);
        Assert.AreEqual(oracle.GetProperty("wp3").GetProperty("hypotheses").GetInt32(), actual.Hypotheses);
        Assert.AreEqual(oracle.GetProperty("wp3").GetProperty("abstentions").GetInt32(), actual.CandidateAbstentions);
        JsonElement wp4 = oracle.GetProperty("wp4");
        Assert.AreEqual(wp4.GetProperty("findings").GetInt32(), actual.Findings);
        Assert.AreEqual(wp4.GetProperty("recommendations").GetInt32(), actual.Recommendations);
        Assert.AreEqual(wp4.GetProperty("supported_cases").GetInt32(), actual.SupportedCases);
        Assert.AreEqual(wp4.GetProperty("lead_only_cases").GetInt32(), actual.LeadOnlyCases);
        Assert.AreEqual(wp4.GetProperty("readiness_effect_from_leads").GetInt32(), actual.LeadReadinessEffects);
        ...
        JsonElement wp5 = oracle.GetProperty("wp5");
        Assert.AreEqual(wp5.GetProperty("publication_commits").GetInt32(), actual.StoredOutputEqual ? 1 : 0);
        Assert.AreEqual(wp5.GetProperty("partial_publications").GetInt32(), actual.StoredOutputEqual ? 0 : 1);
        Assert.AreEqual(wp5.GetProperty("terminal_state").GetString(), actual.StoredRunState);
        Assert.AreEqual(LifecycleState.CompletedWithGaps, actual.LifecycleState);
        Assert.AreEqual(wp5.GetProperty("human_json_semantically_equivalent").GetBoolean(), actual.HumanEmbedsStoredJson);
        Assert.AreEqual(wp5.GetProperty("network_calls").GetInt32(), actual.ExternalEffects);
        Assert.AreEqual(wp5.GetProperty("provider_dispatches").GetInt32(), actual.ExternalEffects);
    }
    ...
    private static ClaimApplicabilityState Applicability(string value) => value switch
    {
        ...
        _ => throw new InvalidDataException("The WP6 applicability value is outside the closed product mapping."),
    };
    ...
    private static string LifecycleStateToken(LifecycleState value) => value switch
    {
        ...
        _ => throw new InvalidDataException("The WP6 product lifecycle is not terminal."),
    };
