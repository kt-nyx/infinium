...
import { checkNexusHealth } from "./nexus/nexusHealth";
import { searchNexusModsByName } from "./nexus/nexusSearch";
...
export const registerIpcHandlers = (): void => {
  ...
  ipcMain.handle(
    "nexus:searchModsByName",
    async (
      _event,
      payload: { game: ProfileSnapshot["game"]; query: string; limit?: number },
    ): Promise<NexusModSearchResult[]> => {
      const settings = await ensureSettings();
      return searchNexusModsByName(settings, payload.game, payload.query, payload.limit);
    },
  );

  ipcMain.handle("nexus:checkHealth", async () => {
    const settings = await ensureSettings();
    return checkNexusHealth(settings);
  });
};
