...
import { enrichModInfoWithMo2Meta, readMo2MetaForMod } from "./mo2Meta";
...
const parseModList = async (instancePath: string, profileId: string): Promise<ModInfo[]> => {
  ...
  for (const line of lines) {
    ...
    const meta = await readMo2MetaForMod(modsDir, name);
    const enriched = meta ? enrichModInfoWithMo2Meta(base, meta) : base;
    mods.push(enriched);
  }
