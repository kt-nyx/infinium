...
const App = () => {
  ...
  const [nexusSearchResults, setNexusSearchResults] = useState<NexusModSearchResult[]>([]);
  ...
  return (
    <div style={{ padding: 24, display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {analysis && analysis.profile.mods.length > 0 && settings.nexusApiKey && (
        <div
          style={{
            marginTop: 8,
            padding: 12,
        ...
        >
          <Text size={200} weight="semibold">
            Nexus ID helper (experimental)
          </Text>
          <Text size={200} style={{ color: "#6b7280" }}>
            Search Nexus Mods by name for the current profile&apos;s mods. Use this to look up the
            correct Nexus page and numeric ID before configuring mod metadata.
          </Text>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <Dropdown
              placeholder="Pick a mod from this profile"
              value={nexusSearchQuery || undefined}
              selectedOptions={nexusSearchQuery ? [nexusSearchQuery] : []}
              style={{ minWidth: 220 }}
              onOptionSelect={(_, data) => {
                if (data.optionValue) {
                  setNexusSearchQuery(String(data.optionValue));
                }
              }}
            >
              {analysis.profile.mods.map((mod) => (
            ...
            <Button
              appearance="secondary"
              disabled={!nexusSearchQuery || nexusSearchLoading}
              onClick={async () => {
                if (!nexusSearchQuery) return;
                try {
                  setNexusSearchLoading(true);
                  const results = await window.api.nexus.searchModsByName(
                    analysis.profile.game,
                    nexusSearchQuery,
                    10,
                  );
                  setNexusSearchResults(results);
                  if (!results.length) {
                    showToast(
                      `No Nexus mods found matching "${nexusSearchQuery}" for ${analysis.profile.game}.`,
                      "warning",
                    );
                  }
                } catch (err) {
                  console.warn("Nexus mod search failed", err);
                  showToast(
                    "Failed to search Nexus Mods. Check your Nexus API key and network connection.",
                    "warning",
                  );
                } finally {
            ...
            >
              {nexusSearchLoading ? "Searching…" : "Search Nexus"}
            </Button>
          </div>
              ...
              {nexusSearchResults.map((mod) => (
                ...
                >
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <Text size={100} style={{ display: "block", color: "#9ca3af", marginTop: 2 }}>
                      {mod.gameDomain} • {mod.downloads.toLocaleString()} downloads •{" "}
                      {mod.endorsements.toLocaleString()} endorsements
                    </Text>
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                    <Button
                      ...
                      appearance="secondary"
                      onClick={() => {
                        void window.api.logs.tail(1); // no-op to keep type-checker happy for now
